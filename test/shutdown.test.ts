...
async function exerciseShutdown(
  ...
  try {
    ...
    if (appServerPid !== undefined && processExists(appServerPid)) {
      process.kill(
        appServerPid,
        process.platform === "darwin" ? "SIGKILL" : undefined,
      );
      const deadline = Date.now() + 3_000;
      while (processExists(appServerPid) && Date.now() < deadline) {
        await delay(20);
...
async function exerciseWindowsManagerClose(stubbornAppServer: boolean): Promise<void> {
  ...
  try {
    ...
    assert.equal(
      ...
      false,
      "Codex app-server must not be orphaned after manager close",
    );
  } finally {
...
test("macOS Bridge process shutdown reaps the exact Codex app-server child", {
  ...
  await t.test("stdin EOF hard-terminates a stubborn child without orphaning it", () =>
